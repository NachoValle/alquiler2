...
@method_decorator(login_required,name='dispatch')
class ListaDeVehiculosDesactivados(ListView):
    model = Vehiculo      
    template_name = 'vehiculo/listadesactivados.html'   
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(ListaDeVehiculosDesactivados, self).get_context_data(**kwargs)
        # Get the blog from id and add it to the context        
        context['vehiculos'] = Vehiculo.objects.filter(activo=False) 
        return context
        
    def dispatch(self,request, *args,**kwargs):
        if request.user.profile.todas == True:
            return super(ListaDeVehiculosDesactivados,self).dispatch(request,*args,**kwargs)
        else:
